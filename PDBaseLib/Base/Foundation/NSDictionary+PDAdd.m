...
@interface _PDXMLDictionaryParser : NSObject <NSXMLParserDelegate>

@end

@implementation _PDXMLDictionaryParser {
...
+ (NSDictionary *)dictionaryWithXML:(id)xml {
    _PDXMLDictionaryParser *parser = nil;
    if ([xml isKindOfClass:[NSString class]]) {
        parser = [[_PDXMLDictionaryParser alloc] initWithString:xml];
    } else if ([xml isKindOfClass:[NSData class]]) {
        parser = [[_PDXMLDictionaryParser alloc] initWithData:xml];
    }
...
@implementation NSMutableDictionary (PDAdd)
